...
void main() {
  ...
  // Radial mask to guarantee 100% transparency at canvas edges
  float edgeMask = smoothstep(0.5, 0.3, length((gl_FragCoord.xy - 0.5 * uResolution.xy) * px));
  c *= edgeMask;

  // Crush near-zero values to eliminate bounding box artifact
  c = max(c - 0.02, 0.0);
